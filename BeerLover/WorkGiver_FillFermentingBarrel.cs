...
namespace BeerLover
{
	public class WorkGiver_FillFermentingBarrel : WorkGiver_Scanner
	{
		...
		public override bool HasJobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			{
				JobFailReason.Is(TemperatureTrans, null);
				return false;
			}
			...
			{
				JobFailReason.Is(NoWortTrans, null);
				return false;
			}
			return !t.IsBurning();
		}
		...
		private Thing FindWort(Pawn pawn, Building_FermentingBarrel barrel)
		{
			...
			if (barrel.Empty)
			{
				foreach (var wortDef in _wortDefs)
				{
					var closestThing = GenClosest.ClosestThingReachable(pawn.Position, pawn.Map, ThingRequest.ForDef(wortDef), PathEndMode.ClosestTouch,
						TraverseParms.For(pawn, Danger.Deadly, TraverseMode.ByPawn, false, false, false), 9999f, validator, null, 0, -1, false, RegionType.Set_Passable, false);
			...
			{
				var containingWortType = barrel.ContainingWortType;
				return GenClosest.ClosestThingReachable(pawn.Position, pawn.Map, ThingRequest.ForDef(containingWortType), PathEndMode.ClosestTouch,
					TraverseParms.For(pawn, Danger.Deadly, TraverseMode.ByPawn, false, false, false), 9999f, validator, null, 0, -1, false, RegionType.Set_Passable, false);
			}
